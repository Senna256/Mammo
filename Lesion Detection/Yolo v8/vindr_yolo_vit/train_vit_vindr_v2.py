...
# Same DICOM image root used by the Swin + YOLOv8 code.
# Layout: NETWORK_IMAGES / study_id / image_id.dicom
NETWORK_IMAGES = Path(
...
# FIND DICOM IMAGE
# ============================================================
def find_image(study_id, image_id, image_root):
    """
    Locate a ViNDR DICOM using the same layout as the Swin + YOLOv8 code:

        image_root / study_id / image_id.dicom
    """
    ...
    if not image_path.exists():
        raise FileNotFoundError(
            f"No s'ha trobat el DICOM: {image_path}"
        )
...
class VinDrViTDataset(Dataset):
    ...
    def __init__(
        ...
        if "study_id" not in self.df.columns:
            raise RuntimeError(
                "Els CSV de ViT han de contenir la columna "
                "'study_id' per poder localitzar els DICOM."
            )
...
def main():
    ...
    # Use exactly the same DICOM root as the Swin + YOLOv8 code.
    # The ViT CSVs still define the train/val/test split, while
    # physical images are read from:
    #
    #   NETWORK_IMAGES / study_id / image_id.dicom
    image_root = NETWORK_IMAGES
